...
func main() {
	...
	for _, sonName := range sonDirList {
		...
		fmt.Println(sonName, ":", grandSonDirList)
	}
	fmt.Println(oneLvlList)
	...
	file, err := os.OpenFile("config.js", os.O_CREATE|os.O_WRONLY|os.O_TRUNC, 0666)
	if err != nil {
		panic(err)
	}
	defer file.Close()
